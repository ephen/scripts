...
`./le-cloudxns.sh [全路径]/cloudxns.conf`
...
`0 0 2/20 * * /etc/nginx/le-cloudxns.sh /etc/nginx/cloudxns.conf >> /var/log/le-cloudxns.log 2>&1`

**注意：** 部分操作系统如`ubuntu 16.04` 不能定义 `day of month` 含有开始天数的 `step values`，可以替换命令中的 `2/20` 为 `2,22`。
